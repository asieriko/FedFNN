...
def read_keel_dat(file_path: str):
    ...
    """
    try:
        with open(f"data/{file_path}/{file_path}.dat", 'r') as file:
        ...
        for col in df.columns[:-1]:
            # To convert ints and floats, and avoid converting categorical columns to numeric
            try:
        ...
        for col in df.columns[:-1]:
            if df[col].dtype == 'object' or df[col].dtype == 'str':  # Check if column is of type object (usually strings)
...
def read_json_dataset(dataset_name: str, dataset_path: Path) -> list[tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    ...
    data_float32 = data.data.astype(np.float32)
    target_int64 = data.target.astype(np.int64)
    data = Dataset(data_float32, data.feature_names, target_int64, data.target_names)

    client_partitions = []
    for i, client in enumerate(partitions.values()):
        train_idx = np.array(client["train"])
        test_idx = np.array(client["test"])
    ...
    return client_partitions, data, partitions
